...
{
  efTask1->set_ef_pose(sva::PTransformd{sva::RotY(-M_PI/2), Eigen::Vector3d{0.5, -0.15, 1.2}});
  auto pt2 = efTask2->get_ef_pose();
  efTask2->set_ef_pose(sva::PTransformd{sva::RotY(-M_PI/2), Eigen::Vector3d{0.5, 0.15, 1.2}});
...
void MyFirstController::reset(const mc_control::ControllerResetData & reset_data)
{
  ...
  //comZero is obtained by doing as forrow
   comZero = comTask->com();
  robots().robot(1).posW(sva::PTransformd(sva::RotZ(M_PI), Eigen::Vector3d(0.6, 0.0, 1.2)));
}
