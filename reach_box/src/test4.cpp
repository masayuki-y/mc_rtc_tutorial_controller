...
{
  efTask1->set_ef_pose(sva::PTransformd{sva::RotY(-M_PI/2), Eigen::Vector3d{0.55, -0.135, 0.55}});
  auto pt2 = efTask2->get_ef_pose();
  efTask2->set_ef_pose(sva::PTransformd{sva::RotY(-M_PI/2), Eigen::Vector3d{0.55, 0.135, 0.55}});
...
void MyFirstController::reset(const mc_control::ControllerResetData & reset_data)
{
  ...
  //comZero is obtained by doing as forrow
   comZero = comTask->com();
  //spawn box at (x,y,z)= (0.65, 0.0, 1.2)
  robots().robot(1).posW(sva::PTransformd(sva::RotZ(M_PI), Eigen::Vector3d(0.65, 0.0, 0.6)));
}
